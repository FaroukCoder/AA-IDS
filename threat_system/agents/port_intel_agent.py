...
class PortIntelAgent(BaseAgent):
    name = "port_intel"
    ...
    def run(self, event: Event, prior_findings: list[AgentReport] | None = None) -> AgentReport:
        ...
        skill = skill_loader.load("port_intel_skill")

        try:
            raw_data = self.tool.fetch(event.ports_targeted)
        ...
        template = _load_prompt("port_intel_agent")
        ...
        system = (
            "You are a port intelligence analyst. "
